...
class BlendForces:
    ...
    def set_lmk_indices(self, indice):
        self.__m_indice = indice
    def set_meshik(self, meshik : mik.MeshIKConstraint):
        """
            set meshik constraint to blendforces.
            meshik : MeshIKConstraint
        """
        self.__m_meshik = meshik
    ...
    def __compute_mass_matrix(self):
        ...
        __m_vn_extended = N + +self.__m_blendshapes.neutral_mesh().f.shape[0]
        diag = np.empty((N*3), dtype=np.float64)
        # diag = np.empty((__m_vn_extended*3), dtype=np.float64)
        diag[...] = self.__m_mass
    ...
    def precompute(self):
        ...
        # normal + vertex size
        self.__m_vN = self.__m_blendshapes.neutral_pose().shape[0]
        self.__m_vn_extended = self.__m_vN + +self.__m_blendshapes.neutral_mesh().f.shape[0]

        self.__m_b = np.empty((3*self.__m_vN,1), dtype=np.float64)
        # self.__m_b = np.empty((3*self.__m_vn_extended,1), dtype=np.float64)
        ...
        func_list = []
        # TODO
        self.__m_G = self.__m_meshik.GMatrix()
        self.__m_reducedG, self.__m_cMatrix, self.__m_compact_reudcedG = self.__m_meshik.getCMatrixAndReducedG(self.__m_marker_indices)
        self.__m_reudcedB = self.__m_meshik.get_reduced_linearized_tw_B(self.__m_marker_indices)
        ...
        column_size = 3*self.__m_vn_extended

        bA = gm.bend_constraint_A( self.__m_blendshapes.neutral_mesh(), self.__m_kb)
        # bA.resize(bA.shape[0], column_size)
        self.__m_precomputed_constraint_Ai[BlendForces.BEND_IDX] = {"coeff" : self.__m_kb, "A" : bA}
        dA = gm.displacement_constraints_A(-self.__m_kp, self.__m_vN)
        # dA.resize(dA.shape[0], column_size)
        self.__m_precomputed_constraint_Ai[BlendForces.DISP_IDX] = {"coeff" : self.__m_kp, "A" : dA}
        sA = gm.stretching_constraint_A(-self.__m_ks, self.__m_vN, self.__m_blendshapes.neutral_mesh().e)
        # sA.resize(sA.shape[0], column_size)
        self.__m_precomputed_constraint_Ai[BlendForces.STRETCH_IDX] = {"coeff" : self.__m_ks, "A" : sA}

        self.__m_As_sum = sp.csc_matrix((self.__m_vN*3, self.__m_vN*3) , dtype=np.float64)
        Grows, Gcols = self.__m_reducedG.shape

        # self.__m_As_sum = sp.csc_matrix((column_size, column_size) , dtype=np.float64)
        ...
        GtG = self.__m_reducedG.T @ self.__m_reducedG + 0.01*sp.identity(Gcols, dtype=np.float64)
        tmp1 = self._tmp1 =  (I -  h2 *  M_inv@ self.__m_As_sum)

        self.__m_G_without_normal = self.__m_meshik.compute_G_from_two_vector()
        self.__m_G_without_v4 = self.__m_meshik.GMatrix_with_v4()

        print("G matrix built without normal vector")
        # phi = tmp1 @ tmp2
        spchol(GtG)
        self.__m_precomputed_I_Asums2 = tmp1
        self.__m_precomputed_I_Asums = spchol(tmp1)

        self.m_examples_extended_v = self.__m_meshik.m_examples_extended_v
        G = self.__m_meshik.GMatrix()
        self.__m_extended_blendshapesB = []
        for i in range(len(self.m_examples_extended_v)):
            self.__m_extended_blendshapesB.append( (self.m_examples_extended_v[i] - self.__m_meshik.extended_v).reshape(-1,1) )
        self.__m_extended_blendshapesB = np.squeeze(np.array(self.__m_extended_blendshapesB)      ).T
    ...
    def update2(self, new_marker_pos, frame, contact_A : sp.csc_matrix, contact_tau_array :np.ndarray, contact_coeff = 50.0):
        ...
        dt_marker_vec = self.__m_cMatrix @ new_marker_pos.reshape(-1, 1)
        ...
        if self.__m_first_iter_flag:    
            ...
            # tmp_dg = self.__m_meshik.Tw(w)
            # self.__x_prev = self.__m_meshik.solve(tmp_dg)
            # self.__x_prev = self.__m_meshik.solve_from_lmk_linear(self.__m_reducedG, self.__m_cMatrix, new_marker_pos, self.__m_indice)
        ...
        h2 = self.__m_step_size**2
        M_inv = self.__m_sp_mass_matrix_inv
        # contact_A.resize(contact_A.shape[0], self.__m_vn_extended*3)
        # tmp1 = self._tmp1 = (I -  h2 * M_inv@ (self.__m_As_sum  +(-contact_coeff*contact_A.T@contact_A)))
        # GtG = self.__m_reducedG.T @ self.__m_reducedG + 0.01 * sp.identity(self.__m_reducedG.shape[1], dtype=np.float64,format="csc")
        tmp1 = self._tmp1 =  (I -  h2 * M_inv@ (self.__m_As_sum  +(-contact_coeff*contact_A.T@ contact_A)))
        # tmp1 = self._tmp1 = self.__m_reducedG @ (I -  h2 * M_inv@ (self.__m_As_sum  +(-contact_coeff*contact_A.T@ contact_A)))
        P = tmp1
        tmp1 = (I -  h2 * M_inv@ (self.__m_As_sum  +(-contact_coeff*contact_A.T@ contact_A)))
        # tmp1 = tmp1.T @ tmp1 + 1e-8 * sp.identity(self.__m_reducedG.shape[1], dtype=np.float64,format="csc")
        # GtG = self.__m_G.T@self.__m_G + 0.01 * sp.identity(self.__m_G.shape[1], dtype=np.float64,format="csc")
        # GtG = self.__m_compact_reudcedG.T@self.__m_compact_reudcedG + 0.01 * sp.identity(self.__m_compact_reudcedG.shape[1], dtype=np.float64,format="csc")
        GtG = self.__m_compact_reudcedG.T@self.__m_compact_reudcedG 
        # GtG = self.__m_G.T@self.__m_G + 0.01 * sp.identity(self.__m_G.shape[1], dtype=np.float64,format="csc")
        # tmp1 = self._tmp1 = (I -  h2 * M_inv @ (self.__m_As_sum))

        self.__m_precomputed_I_Asums = spchol(tmp1)

        # tmp_non= h2 * self.__m_reducedG.T @ self.__m_reducedG
        # self.nonlinear_phi = self.__m_precomputed_I_Asums(tmp_non)
        # non_phi = self.__m_precomputed_I_Asums(        (h2 * P.T).tocsc())

        x_prev_marker = self.__m_blendshapes.make_pose_by_weight(w)[self.__m_indice, :]
        nonlin_B_dot_ut = self.__m_meshik.Tw(w).reshape(-1,1)
        x_t_tmp = spsolve(GtG, self.__m_compact_reudcedG.T@(nonlin_B_dot_ut - self.__m_cMatrix@x_prev_marker.reshape(-1,1))) 
        # x_t_tmp = spsolve(self.__m_G.T@self.__m_G, self.__m_G.T@(nonlin_B_dot_ut ))

        xidx = np.setdiff1d( np.arange(self.__m_vN), self.__m_indice )

        Buu = np.zeros((self.__m_vN,3), dtype=np.float64)
        x_t_tmp = x_t_tmp.reshape(-1,3)[:-self.__m_blendshapes.neutral_mesh().f.shape[0],:]
        Buu[xidx, :] =  x_t_tmp

        # x_t=  x_t_tmp
        Buu[self.__m_indice,:] = x_prev_marker

        frames = []
        x_t = self.__x_prev + self.__m_step_size* self.__x_acc_prev

        # target_x_t = self.__m_meshik.solve_nonlinear_from_lmk(self.__m_compact_reudcedG, self.__m_cMatrix, new_marker_pos, self.__m_indice)
        # target_x_t -= self.__m_blendshapes.neutral_pose()
        for f in range(frame):
            for iter_n in range(self.__m_iteration_num):
                gm.linearize_force_nonlinear(self.__m_b, x_t[:self.__m_vN, :], self.__m_kp, self.__m_ks,self.__m_kb, \
                ...
                # self.phi, self.y_t =gm.solve_phi_yt_nonlinear(self.__m_reducedG, self.__m_b, self.__m_reudcedB, self.__m_sp_mass_matrix_inv, self.__m_precomputed_I_Asums, self.__x_prev, self.__x_acc_prev, self.__m_step_size)
                ...
                # nonlinear_phi =   self.__m_precomputed_I_Asums(h2 * M_inv @ B)

                # Gxt = self.__m_reducedG @ x_t.reshape(-1,1)
                # Gxtt = self.__m_cMatrix @ x_t[self.__m_indice ,...].reshape(-1,1)
                # Gxt32 = Gxtt + Gxt
                # Gx2 = self.__m_G @ x_t.reshape(-1,1)
                # tet = Gxt + dt_marker_vec
                # self.phi =    self.__m_G @ self.phi
                # self.y_t =   self.__m_G @ self.y_t.reshape(-1,1)
                # self.phi =    self.__m_reducedG @ self.phi
                # self.y_t =   self.__m_reducedG @ self.y_t.reshape(-1,1)
                u_t = gm.solve_ut(self.__M_S_sp_mat, self.phi, self.y_t, new_marker_pos)

                # u_t = gm.solve_ut_nonlinear(Gxt, self.phi, self.y_t, dt_marker_vec)

                # xt = self.__m_meshik.solve_from_lmk_linear(self.__m_reducedG, self.__m_cMatrix, new_marker_pos, self.__m_indice)
                # break

                nonlin_B_dot_ut = self.__m_meshik.Tw(u_t)
                # nonlin_B_dot_ut = self.__m_meshik.Tw( np.clip( u_t, 0.0, 1.0) )

                nonlin_B_dot_ut = nonlin_B_dot_ut.reshape(-1,1)
                # bot = self.__m_precomputed_I_Asums(P.T@(nonlin_B_dot_ut + self.y_t) )
                # nonlinear_phi = tmp_non @ nonlin_B_dot_ut.reshape(-1,1)

                # b = self.__m_step_size**2 * nonlin_B_dot_ut + self.__m_G_without_v4 @ (self.__x_prev.reshape(-1,1) + self.__x_acc_prev.reshape(-1,1) * self.__m_step_size + self.__m_step_size**2 *M_inv @ self.__m_b)
                # x_t_tmp = spsolve(GtG, self.__m_compact_reudcedG.T@(nonlin_B_dot_ut - dt_marker_vec))
                x_prev_marker = self.__m_cMatrix@self.__x_prev[self.__m_indice, :].reshape(-1,1)
                # x_t_tmp = spsolve(GtG, self.__m_compact_reudcedG.T@(nonlin_B_dot_ut - dt_marker_vec))
                x_prev_marker = self.__m_blendshapes.make_pose_by_weight(u_t)[self.__m_indice, :]
                x_t_tmp = spsolve(GtG, self.__m_compact_reudcedG.T@(nonlin_B_dot_ut - self.__m_cMatrix@x_prev_marker.reshape(-1,1))) 
                # x_t_tmp = spsolve(self.__m_G.T@self.__m_G, self.__m_G.T@(nonlin_B_dot_ut ))

                xidx = np.setdiff1d( np.arange(self.__m_vN), self.__m_indice )

                x_t = np.zeros((self.__m_vN,3), dtype=np.float64)
                x_t_tmp = x_t_tmp.reshape(-1,3)[:-self.__m_blendshapes.neutral_mesh().f.shape[0],:]
                x_t[xidx, :] =  x_t_tmp
                # x_t=  x_t_tmp
                x_t[self.__m_indice,:] = x_prev_marker
                # target_x_t = x_t - self.__m_blendshapes.neutral_pose()

                target_x_t = Buu - self.__m_blendshapes.neutral_pose()
                # x_t = self.__m_meshik.solve_nonlinear_from_lmk(self.__m_compact_reudcedG, self.__m_cMatrix, new_marker_pos, self.__m_indice)
                # x_t = self.__m_meshik.solve_nonlinear_from_lmk(self.__m_compact_reudcedG, self.__m_cMatrix, new_marker_pos, self.__m_indice, u_t, 1, 10)
                # x_t[self.__m_indice,:] =  new_marker_pos.reshape(-1,3)
                # x_t -= self.__m_blendshapes.neutral_pose()
                # test
                # break
                # Bu = x_t.reshape(-1,1)
                Bu = target_x_t.reshape(-1,1) 
                b = self.__m_step_size**2 * M_inv @ (Bu) + self.__x_prev.reshape(-1,1) + self.__x_acc_prev.reshape(-1,1) * self.__m_step_size + self.__m_step_size**2 *M_inv @ self.__m_b
                x_t = self.__m_precomputed_I_Asums(b)

                x_t, x_acc_t = gm.simulate_time_step_nonlinear(self.__x_prev, x_t, self.__m_step_size)
            ...
            new_xt = x_t
            # nonlin_B_dot_ut = self.__m_meshik.Tw(u_t)
            # x_t_tmp = spsolve(GtG, self.__m_compact_reudcedG.T@(nonlin_B_dot_ut.reshape(-1,1) - self.__m_cMatrix@new_marker_pos.reshape(-1,1)))
            # xidx = np.setdiff1d( np.arange(self.__m_vN), self.__m_indice )
            # x_t_tmp = x_t_tmp.reshape(-1,3)[:-self.__m_blendshapes.neutral_mesh().f.shape[0],:]
            # x_t[xidx, :] =  x_t_tmp
            # x_t[self.__m_indice,:] =new_marker_pos
            # new_xt= x_t

            # new_xt = new_xt[:self.__m_vN]
            # frames.append(new_xt)

            # new_xt = sp.linalg.spsolve(self.__m_reducedG.T@self.__m_reducedG, self.__m_reducedG.T @ self.__m_reducedG   @ x_t.reshape(-1,1)).reshape(-1,3)
            # new_xt = new_xt[:self.__m_vN]
            frames.append(new_xt)
        return frames
...
if __name__ == "__main__":
    ...
    bb.set_lmk_indices(lmk_idx)
    bb.set_meshik(meshik)
    bb.precompute()
    xt = neutral[lmk_idx]
    # for it in range(50) : 
